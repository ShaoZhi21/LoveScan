...
export const ScreenshotAnalysisScreen = (): JSX.Element => {
  ...
  useEffect(() => {
    const analyzeChatScreenshots = async () => {
      try {
        ...
        // Get the latest chat scan with screenshots
        ...
        if (scanError) {
          if (scanError.code === 'PGRST116') {
            throw new Error('No screenshot found to analyze. Please upload a screenshot first.');
          }
          throw scanError;
        }
        ...
        // Call the screenshot analysis function with the session token
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData?.error || 'Screenshot analysis failed. Please try again.');
        }
        ...
        // Transform the analysis results into chat messages
        const chatMessages: ChatMessage[] = analysis.messages.map((msg: any, index: number) => ({
          id: String(index + 1),
          text: msg.text,
          isUser: index % 2 === 0, // Alternate between user and other person
  ...
  return (
    <div className="bg-[#8b0000] min-h-screen flex flex-col">
      ...
      <div className="h-[60px] flex items-center justify-between px-4">
        <h1 className="text-xl font-bold text-white">Screenshot Chat Analysis</h1>
      </div>
      ...
      <div className="flex-1 bg-white rounded-t-[20px] p-4">
        {isLoading ? (
          <div className="flex items-center justify-center h-48">
            <div className="text-gray-600">Analyzing chat...</div>
          </div>
        ) : error ? (
